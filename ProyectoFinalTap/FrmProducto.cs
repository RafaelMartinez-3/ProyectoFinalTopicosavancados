...
namespace ProyectoFinalTap
{
    ...
    public partial class FrmProducto : MetroForm
    {
        ...
        private void btnAceptar_Click_1(object sender, EventArgs e)
        {
            ...
            {
                if (txtNombre.Text.Length==0 || txtNombre.Text.Length>40)
                {
                    MessageBox.Show("Nombres solo acepta entre 1 y 40 caracteres.");
                }
                else if (precioUnidad<=0)
                {
                    MessageBox.Show("El precio de la unidad debe ser mayor que 0.");
                }
                else if (unidadesStock<=0 || unidadesStock > 65534)
                {
                    MessageBox.Show("Unidades en stock debe ser mayor que 0 y menor que 65534.");
                }
                else if (reorder<=0 || reorder>65534)
                {
                    MessageBox.Show("El nivel de reorder debe ser mayor que 0 y menor que 65534.");
                }
                else
                {
                    if (proId == 0)
